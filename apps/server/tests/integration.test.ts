...
function waitForEvent(socket: ClientSocket, timeoutMs = 1500): Promise<Record<string, unknown>> {
	return new Promise((resolve, reject) => {
		const timer = setTimeout(() => reject(new Error("timeout waiting for realtime event")), timeoutMs);
...
describe("realtime pipeline (integration)", () => {
	...
	it("delivers a signed event to an authenticated client of the target audience", async () => {
		...
		const response = await postEmit(server.port, baseEnvelope({ audience: { tenantId: "7", userIds: ["12"] } }));
	...
	it("delivers entity-room events only while subscribed", async () => {
		...
		await new Promise<void>((resolve, reject) => {
			client.emit(SUBSCRIBE_EVENT, { entityType: "order", entityId: 123 }, (ack: { ok: boolean }) =>
				ack.ok ? resolve() : reject(new Error("subscribe ack failed")),
			);
		});

		const firstEventPromise = waitForEvent(client);
		await postEmit(server.port, baseEnvelope({ audience: { tenantId: "7", entity: { type: "order", id: 123 } } }));
		await expect(firstEventPromise).resolves.toMatchObject({ type: "order.updated" });

		await new Promise<void>((resolve, reject) => {
			client.emit(UNSUBSCRIBE_EVENT, { entityType: "order", entityId: 123 }, (ack: { ok: boolean }) =>
