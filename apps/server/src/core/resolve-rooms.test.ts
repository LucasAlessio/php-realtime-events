...
describe("resolveRooms", () => {
  ...
  it("resolves to one room per userId when userIds are given", () => {
    expect(resolveRooms({ tenantId: 7, userIds: [12, 13] })).toEqual([
      "tenant:7:user:12",
      "tenant:7:user:13",
    ]);
  });
  ...
  it("narrows to only the entity room when entity is given without userIds", () => {
    expect(resolveRooms({ tenantId: 7, entity: { type: "order", id: 123 } })).toEqual([
      "tenant:7:order:123",
    ]);
  });
  ...
  it("adds the entity room alongside per-user rooms", () => {
    expect(
      resolveRooms({ tenantId: 7, userIds: [12], entity: { type: "order", id: 123 } }),
    ).toEqual(["tenant:7:user:12", "tenant:7:order:123"]);
  });

  it("always scopes rooms by tenant, never producing a global room", () => {
    const rooms = resolveRooms({ tenantId: "acme", userIds: [1], entity: { type: "x", id: 1 } });
